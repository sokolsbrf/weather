...
public class WeatherService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        //пока просто вызываем метод для проверки
        getLocation();
    ...
    public void getLocation() {
        ...
        Location lastBestLocation = getLastBestLocation(providers, locationManager);

        if (lastBestLocation != null) {
            //берем старые координаты и используем их
        } else {
            //нужно брать текущие координаты
        }
    ...
    private Location getLastBestLocation(List<String> providers, LocationManager lm) {
        ...
        if (result.size() > 0) {
            ...
            Collections.sort(result, new Comparator<android.location.Location>() {
                @Override
